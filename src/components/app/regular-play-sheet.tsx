...
export function RegularPlaySheet({ post, currentUserId, onClose, onChange, onCancelled, messages, currentUser }: RegularPlaySheetProps) {
    ...
    const handleConnect = useCallback(async () => {
        ...
        if (rpcError) {
            setError(describeActionError(rpcError, "send that request"));
            return;
        }
        if (!data.success) {
    ...
    const handleCancel = useCallback(async () => {
        ...
        if (rpcError || !data?.success) {
            setCancelling(false);
            setError(describeActionError(rpcError, "cancel that request"));
            return;
        }
    ...
    return (
        ...
        >
            {/* The blur lives on the backdrop, not on the container that holds the
                sheet. iOS puts the caret in the wrong place when a text field has a
                backdrop-filter ancestor — it drew below the disclaimer while the
                field kept its placeholder. Same look: this layer is what sits over
                the page. */}
            <div
            ...
            >
                ...
                <div className={cx("flex flex-col gap-3", !isOwnPost && !postClosed && "mt-4")}>
                    {isOwnPost ? (
                        ...
                        <>
                            {/* Brand green, like every other primary action. This was
                                blue to match the card's left accent bar, which still
                                identifies regular play — but the accent bar is the
                                identifier, and the primary button is the primary
