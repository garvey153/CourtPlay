-- Surface play_type + duration in the Activity RPCs.
-- The 20260620000000 migration added play_type/duration and updated get_feed +
-- get_post_by_id, but missed get_my_posts_with_claims / get_my_claims_with_posts.
-- Since sub_need posts now store their type in play_type (format stays null) and
-- claims only attach to sub_need posts, the Activity tabs were labelling every
-- post/claim with the generic "Sub needed" fallback. Add the two keys so the
-- client can prefer play_type over format.
